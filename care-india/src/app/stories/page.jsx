...
export default function StoriesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data?.ok) setItems(data.stories || []);
      } catch (err) {
  ...
  const borderGlow = `${emeraldMid}33`;
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="relative">
        <div className="w-full h-64 md:h-[380px] lg:h-[460px] overflow-hidden">
          <img src="/images/casesHero.jpg" alt="Stories Hero" className="w-full h-full object-cover" />
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 mt-4 rounded-xl" style={{ boxShadow: `0 10px 30px ${borderGlow}, 0 6px 18px ${saffronMid}22`, background: "white" }}>
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">Home &gt; Stories &gt; <span className="font-medium text-gray-800">Stories</span></div>
            <div className="text-sm text-gray-500 uppercase tracking-wide">Stories</div>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto text-center py-8 px-4">
          ...
          <p className="text-gray-700 text-base md:text-lg">Real stories from our work — click any card to read the full story.</p>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-20">
        {loading ? (
          ...
          <motion.div initial="hidden" animate="visible" variants={containerVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {items.map((it, idx) => {
              const slug = encodeURIComponent(it.slug || it._id);
              return (
                <motion.article
                  key={it._id || idx}
                ...
                >
                  <Link href={`/stories/${slug}`} className="group block flex-1 flex flex-col" aria-label={`Read story: ${it.title}`}>
                    <div className="w-full h-48 md:h-56 lg:h-64 bg-gray-100 overflow-hidden relative" style={{ borderBottom: `4px solid rgba(0,0,0,0.02)` }}>
                      {it.cardImage ? (
                        <img src={it.cardImage} alt={it.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                      ) : (
